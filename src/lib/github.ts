...
export function parseGitHubUrl(rawUrl: string): { owner: string; repo: string } {
  ...
  // Anti-DoS : URL trop longue (constante partagée avec le client)
  if (trimmed.length > MAX_GITHUB_URL_LENGTH) {
    throw new Error(`URL trop longue (max ${MAX_GITHUB_URL_LENGTH} caractères).`);
  }
...
export async function fetchRepoContent(githubUrl: string): Promise<RepoData> {
